...
import org.wahlzeit.model.turtle.*;
...
public class NotifyUsersAboutPraiseAgent extends Agent {
	...
	protected void doRun() {
		Map<PhotoId, Photo> photoCache = TurtlePhotoManager.getInstance().getPhotoCache();
		...
		for (Photo photo : photos) {
			if (photo != null && photo.isVisible() && photo.hasNewPraise()) {
				...
				if (ownerId != null) {
					...
					TurtlePhotoManager.getInstance().savePhoto(photo);
				}
			}
		}
